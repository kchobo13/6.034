...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1,Sum) == True and isinstance(expr2,Sum) == True:
        final_list = []
        for term1 in expr1:
            for term2 in expr2:
                final_list.append(Product([term1,term2]))
        return Sum(final_list)

    elif isinstance(expr1,Sum) == True and isinstance(expr2,Product) == True:
        final_list = []
        for term in expr1:
            list2 = list(expr2)
            list2.append(term)
            insert = Product(list2)
            final_list.append(insert)
        return Sum(final_list)

    elif isinstance(expr2,Sum) == True and isinstance(expr1,Product) == True:
        final_list = []
        for term in expr2:
            list1 = list(expr1)
            list1.append(term)
            insert = Product(list1)
            final_list.append(insert)
        return Sum(final_list)

    elif isinstance(expr1,Product) == True and isinstance(expr2,Product) == True:
        list1 = list(expr1)
        list2 = list(expr2)
        for term in list1:
            list2.append(term)
